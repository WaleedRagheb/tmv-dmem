...
def get_prediction2(x):
    if is_listy(x): x2=x[0]
    attScr = x[3]
    return x2.data, attScr.data
...
def predict2(m, dl):
    preda = predict_with_targs_2(m, dl)
    return preda
...
def predict_with_targs_2(m, dl):
    m.eval()
    if hasattr(m, 'reset'): m.reset()
    res = []
    scores = []
    for *x,y in iter(dl):
        rs_pr, res_sc = get_prediction2(m(*VV(x)))
        res.append(rs_pr)
        scores.append(res_sc)
    return res, scores
