...
def precision_eRisk(preds, targs):
    preds = torch.max(preds, dim=1)[1]
    metrics = precision_recall_fscore_support(targs, preds, average=None)
    return metrics[0][1]

def recall_eRisk(preds, targs):
    preds = torch.max(preds, dim=1)[1]
    metrics = precision_recall_fscore_support(targs, preds, average=None)
    return metrics[1][1]

def f1_eRisk(preds, targs):
    preds = torch.max(preds, dim=1)[1]
    metrics = precision_recall_fscore_support(targs, preds, average=None)
    f = metrics[2]
    return f[len(f)-1]

#def f1_sklearn(y_pred:torch.Tensor, y_true:torch.Tensor):
#    y_pred = y_pred.max(1)[1]
#
#    res = f1_score(y_true, y_pred, average=None)
#    return torch.Tensor([res[1]]).float().squeeze()
#