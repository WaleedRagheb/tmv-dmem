...
def train_clas(dir_path, cuda_id, lm_id='', clas_id=None, bs=64, cl=1, backwards=False, startat=0, unfreeze=True,
               ...
               from_scratch=False, train_file_id='', use_att=False, useWeightSampler = False):
    ...
    if useWeightSampler:
        wl2i = []
        l2p = [0.1, 0.9]
        for ind, d in enumerate(tqdm(trn_ds)):
            wl2i.append(l2p[d[1]])
        trn_samp = WeightedRandomSampler(wl2i, len(trn_ds), replacement=True)
        pass
    else:
    ...
    if use_att:
       # m = get_rnn_classifier_w2(bptt, 20 * 70, c, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
       #                          layers=[em_sz, 100, c], drops=[dps[4], 0.1],
       #                           dropouti=dps[0], wdrop=dps[1], dropoute=dps[2], dropouth=dps[3])
        m = get_rnn_classifier_w_v2(bptt, 20 * 70, c, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
    ...
    learn.clip=25.
    #learn.metrics = [accuracy, precision_eRisk, recall_eRisk, f1_eRisk]
    #learn.metrics = [accuracy]
    learn.metrics = [accuracy, f1_eRisk]
    lrm = 2.6
    ...
    if use_regular_schedule:
        ...
        n_cycles = 1
    #learn.fit(lrs, n_cycles, wds=wd, cycle_len=cl, use_clr=(32,10) if use_clr else None)
    learn.fit(lrs, n_cycles, wds=wd, best_save_name=str(final_clas_file + '_BEST'), cycle_len=cl, use_clr=(32, 10) if use_clr else None)
    print('Plotting lrs...')
    learn.sched.plot_lr()
...
def train_clas_eRisk(dir_path, cuda_id, lm_id='', clas_id=None, bs=64, cl=1, backwards=False, startat=0, unfreeze=True,
               lr=0.01, dropmult=1.0, bpe=False, use_clr=True,
               use_regular_schedule=False, use_discriminative=True, last=False, chain_thaw=False,
               from_scratch=False, train_file_id='', use_att=False, useWeightSampler = False):
    print(f'dir_path {dir_path}; cuda_id {cuda_id}; lm_id {lm_id}; clas_id {clas_id}; bs {bs}; cl {cl}; backwards {backwards}; '
    ...
    if (startat<1) and not last and not chain_thaw and not from_scratch:
        learn.freeze_to(-1)
        learn.fit(lrs, 1, wds=wd, cycle_len=None if use_regular_schedule else 3,
                  use_clr=None if use_regular_schedule or not use_clr else (8,3))
