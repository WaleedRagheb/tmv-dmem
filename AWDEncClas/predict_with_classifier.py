...
def load_model(itos_filename, classifier_filename):
    ...
    #model = get_rnn_classifier_w_ML(bptt, 20*70, num_classes, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
    #        layers=[em_sz*3, 50, num_classes], drops=[0., 0.])

    model = get_rnn_classifier_w(bptt, 20 * 70, num_classes, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
                        layers=[em_sz*3, 50, num_classes], drops=[0., 0.])
...
def predict_text(stoi, model, text):
    ...
    tensor = torch.from_numpy(ary)#.long().to(device)
    ...
    #return softmax(numpy_preds[0])[0], predictions[3]
    return numpy_preds[0], predictions[3], tok
...
def predict_textIn(itos_filename, trained_classifier_filename, textToclass):
    ...
    scores, att_scores, tok = predict_text(stoi, model, textToclass)
    classes = [False, True]
    print("Result: {0}, Scores: {1}".format(classes[np.argmax(scores)], scores))
    rtrn_txt = "~~".join(map(str, tok[0]))
    #return classes[np.argmax(scores)], att_scores.data.numpy(), rtrn_txt
    return classes[np.argmax(scores)], att_scores, rtrn_txt
