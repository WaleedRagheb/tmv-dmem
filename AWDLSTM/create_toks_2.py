...
def get_texts_eRisk(df, n_lbls):
    ...
    print('Hi, Im not using partitioning.........')
...
def get_all_eRisk_2(df, n_lbls):
    tok, labels = [], []
    for i, r in df.iterrows():
        #print(i)
        r_df = pd.DataFrame({0: [r['labels']], 1: [r['text']]})
        tok_, labels_ = get_texts_eRisk(r_df, n_lbls)
        tok += tok_;
        labels += labels_
    return tok, labels
...
def create_toks(prefix, pr_abbr, chunksize=24000, n_lbls=1):
    ...
    df_trn = pd.read_csv(f'{PATH}train.csv', header=None, chunksize=chunksize, engine='python')
    df_val = pd.read_csv(f'{PATH}test.csv', header=None, chunksize=chunksize, engine='python')
