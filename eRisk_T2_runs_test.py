...
def get_texts_final(path):
    ...
    for fname in (path).glob('*.*'):
        ...
        ff = fname.open('r', encoding='utf-8')
        txt = ff.read()
...
def logRun(runIdx,DecisionDir, uniq_subID, prediction, scores, maxRound):
    firstRound = 0 # we can change it to [zero] if we want to start from the initial sequence
...
def eRisk_test_runs(writingDir, DecisionDir):
...
#
   ...
   #tok_val, val_labels = get_all_eRisk(df_val1, 1)
    ...
    np.save(CLAS_PATH+'/'+'tmp'+'/'+'tests'+'/'+'test'+'/'+'val_ids.npy', val_clas)

    create_bw_data_test(CLAS_PATH+'/'+'tmp'+'/'+'tests'+'/'+'test'+'/', LM_PATH+'/'+'tmp'+'/'+'itos.pkl')

    print('Start running the models .... ')

    prediction_fw, samplIdx_fw, pos_prob_fw = eval_clas_eRisk_final_2(str(CLAS_PATH), 3, lm_id=lm_id, clas_id=clas_id, attention=False)
    prediction_bw, samplIdx_bw, pos_prob_bw = eval_clas_eRisk_final_2(str(CLAS_PATH), 3, lm_id=lm_id, clas_id=clas_id, attention=False, backwards=True)
...
if __name__ == "__main__":
    eRisk_test_runs('Java/Data/test/writings', 'Java/Data/test/Decisions')